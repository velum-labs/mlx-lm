"""Validation for local model-fusion contract fixtures.

Kept import-safe (no ``mlx`` import) so protocol tooling can validate fixtures
without loading the inference stack. Split out of ``openai_compat`` so that the
OpenAI tool-call helpers and the model-fusion contract validators no longer live
in one module with two unrelated responsibilities.
"""
