"""OpenAI tool-call formatting and validation helpers for the HTTP server.

Model-fusion contract validation lives in :mod:`mlx_lm.model_fusion_contracts`;
the contract validators are re-exported here for backwards compatibility with
existing imports (``server_metadata``, scripts, benchmarks, tests).
"""
